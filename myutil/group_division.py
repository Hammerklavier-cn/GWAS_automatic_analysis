...
def divide_pop_by_ethnic(
    ...
    for col_name in eth_info.columns:
        if re.match(pattern, col_name, re.IGNORECASE):
            ...
            eth_info.rename({col_name: "eth_info_eth_coding"})
            break
    else:
    ...
    ethnic_names = set(eth_ref.select("meaning").to_series().to_list())
